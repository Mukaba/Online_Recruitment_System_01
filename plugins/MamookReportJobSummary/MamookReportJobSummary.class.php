...
/*
 ...
 +------------------------------------------------------------------------------+
 | Filename: MamookReportJobSummary.class.php                                   |
 +------------------------------------------------------------------------------+
 | Description:                                                                 |
 +------------------------------------------------------------------------------+
*/
...
class MamookReportJobSummary extends PluginInterface {

    var $select = 'job_summary';

    function MamookReportJobSummary() {
        $this->plugin_version_number = '1.0';
        $this->plugin_author = 'University of Victoria';
        $this->plugin_name = 'Job Summary Report';
    ...
    function handleTraffic($select) {
        ...
        if ($select == $this->getSelect()) {
            if ($auth->userlevel == OFFICE || $auth->userlevel == FACULTY) {
                include($plugin_absolute_path.'job_summary.inc');
                return true;
            }
